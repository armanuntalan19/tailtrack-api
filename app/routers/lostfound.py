...
def update_report(report_id: int, data: schemas.LostFoundUpdate, payload: dict = Depends(get_current_user)):
    db = SessionLocal()
    try:
        report = db.query(models.LostFoundReport).filter(models.LostFoundReport.id == report_id).first()
        if not report:
...
def delete_report(report_id: int, payload: dict = Depends(get_current_user)):
    db = SessionLocal()
    try:
        report = db.query(models.LostFoundReport).filter(models.LostFoundReport.id == report_id).first()
        if not report:
